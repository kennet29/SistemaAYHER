...
const ALIGNMENT_LEFT: Partial<ExcelJS.Alignment> = { horizontal: 'left', vertical: 'middle' };
...
export type InvoiceExcelData = {
  ...
  metodo: any;
  numeroFactura: string; // Numero de factura principal
  numerosFacturaAdicionales?: string[]; // Numeros adicionales
  items?: {
    cantidad: number;
    numeroParte: string;
...
export async function generateInvoiceExcel(data: InvoiceExcelData): Promise<Buffer> {
