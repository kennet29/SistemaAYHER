...
type Marca = { nombre?: string | null } | null;
type Inventario = { nombre?: string | null; numeroParte?: string | null; marca?: Marca } | null;
...
type DevolucionVenta = {
  ...
  venta?: {
    ...
    cliente?: { nombre?: string | null; empresa?: string | null; ruc?: string | null; direccion?: string | null } | null;
    tipoCambioValor?: number | null;
  } | null;
};
...
export function generarNotaCreditoVentaPDF(res: Response, payload: { config: Config | null; devolucion: DevolucionVenta; detalles: Detalle[] }) {
  ...
  const cliente =
    devolucion.cliente ||
    (devolucion.venta?.cliente as any)?.empresa ||
    devolucion.venta?.cliente?.nombre ||
    "-";
  const consecutivo = devolucion.venta?.numeroFactura || "Sin factura";
  const clienteEntidad = devolucion.venta?.cliente as any;
  ...
  // Header
  const logoPath = resolveLogoPath(config?.logoUrl || undefined);
  if (logoPath) {
  ...
  const rowHeight = 18;
  const columns = [
    { header: "Codigo", width: 58, align: "center" as const },
    { header: "Cant.", width: 45, align: "center" as const },
    { header: "Articulo", width: 165, align: "center" as const },
    { header: "Precio C$", width: 65, align: "right" as const },
    { header: "Precio US$", width: 65, align: "right" as const },
    { header: "Subtotal C$", width: 68, align: "right" as const },
    { header: "Subtotal US$", width: 65, align: "right" as const },
  ];
  ...
  const drawRow = (y: number, values: string[], isHeader = false, fill = false) => {
    if (isHeader) {
  ...
  const drawVerticals = (startY: number, endY: number) => {
    xPositions.slice(0, -1).forEach((x) => {
      doc.moveTo(x, startY - 3).lineTo(x, endY + 0).stroke();
    });
  };

  const totalCordoba = detalles.reduce(
