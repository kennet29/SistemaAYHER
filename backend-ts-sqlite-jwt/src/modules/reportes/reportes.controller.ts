...
// ===============================
export async function clienteDetalle(req: Request, res: Response) {
  try {
    const clienteId = Number(req.params.clienteId);
    if (isNaN(clienteId)) return res.status(400).json({ message: 'ID de cliente invalido' });
    ...
    const productosMap = new Map<number, { nombre: string; cantidad: number; veces: number; total: number }>();
    const marcasMap = new Map<string, { cantidad: number; veces: number }>();
    const categoriasMap = new Map<string, { cantidad: number; veces: number }>();
...
// ===============================
export async function ventasDetalladasClientes(req: Request, res: Response) {
  try {
