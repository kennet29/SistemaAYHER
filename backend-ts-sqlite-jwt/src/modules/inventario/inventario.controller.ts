...
const InvCreateSchema = z.object({
  ...
  descontinuado: z.boolean().optional().default(false),
...
export async function create(req: Request, res: Response) {
  try {
    ...
    if (codigoSustiRaw && marcaSustiCreate && !esMultiple) {
      ...
      const sustitutoExiste = await prisma.inventario.findFirst({
        where: {
    ...
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(data, "codigoSustituto")) {
        await prisma.$executeRawUnsafe(
          'UPDATE "Inventario" SET codigoSustituto = ?, marcaSustitutoId = ? WHERE id = ?',
          codigoSustiPlano,
          marcaSustiPlano,
          id
        );
...
export async function listLowStock(_req: Request, res: Response) {
  try {
    ...
    const filtered = items.filter((i: any) => {
      ...
      if (typeof i.stockMinimo === 'number' && stockActual <= stockMinimo) return true;
...
export async function listLowStockExcel(_req: Request, res: Response) {
  try {
    const items = await prisma.inventario.findMany({
      include: {
        marca: true,
