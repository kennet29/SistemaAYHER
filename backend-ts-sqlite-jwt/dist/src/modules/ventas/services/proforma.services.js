...
async function generarProformaPDFStreamV3({ empresa, cliente, detalles, totalCordoba, totalDolar, tipoCambio, pio, incoterm, plazoEntrega, condicionPago, proformaId, moneda }, res) {
    const doc = new pdfkit_1.default({ size: "A4", margin: 40 });
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="proforma.pdf"`);
    doc.pipe(res);
    const esUSD = moneda === "USD";
    const fmtMoney = (n) => esUSD ? `$ ${Number(n || 0).toFixed(2)}` : `C$ ${Number(n || 0).toFixed(2)}`;
    const pageWidth = doc.page.width;
    const pageHeight = doc.page.height;
    const left = doc.page.margins.left;
    const right = pageWidth - doc.page.margins.right;
    ...
    const drawHeader = () => {
        ...
        if (logoPath) {
            try {
                doc.image(logoPath, left, headerTop, { width: 200 });
            }
            catch { }
        }
