...
exports.generarProformaPDF = generarProformaPDF;
exports.generarProformaPDFStream = generarProformaPDFStream;
exports.generarProformaPDFStreamV2 = generarProformaPDFStreamV2;
exports.generarProformaPDFStreamV3 = generarProformaPDFStreamV3;
const pdfkit_1 = __importDefault(require("pdfkit"));
const fs_1 = __importDefault(require("fs"));
const logo_1 = require("../../../utils/logo");
const normalizePio = (pio) => {
    return typeof pio === "string" && pio.trim().length > 0 ? pio.trim() : "N/A";
};
async function generarProformaPDF({ empresa, cliente, detalles, totalCordoba, totalDolar, pio }) {
    ...
    ["Parte", "Nombre", "Cant", "Precio", "Subtotal"].forEach((txt, i) => doc.text(txt, colX[i], tableTop));
...
async function generarProformaPDFStream({ empresa, cliente, detalles, totalCordoba, totalDolar, tipoCambio, pio }, res) {
    const doc = new pdfkit_1.default({ size: "A4", margin: 40 });
    ...
    const truncate = (text, maxWidth) => {
        if (!text)
            return "";
        let t = String(text);
        while (doc.widthOfString(t) > maxWidth && t.length > 1)
            t = t.slice(0, -1);
        if (t.length < String(text).length)
...
async function generarProformaPDFStreamV2({ empresa, cliente, detalles, totalCordoba, totalDolar, tipoCambio, pio }, res) {
    const doc = new pdfkit_1.default({ size: "A4", margin: 40 });
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="proforma.pdf"`);
    doc.pipe(res);
    const fmtNIO = (n) => {
        try {
            return new Intl.NumberFormat('es-NI', { style: 'currency', currency: 'NIO', minimumFractionDigits: 2 }).format(Number(n || 0));
        }
        catch {
    ...
    const fmtUSD = (n) => {
        try {
            return new Intl.NumberFormat('es-NI', { style: 'currency', currency: 'USD', minimumFractionDigits: 2 }).format(Number(n || 0));
        }
        catch {
    ...
    if (logoPath) {
        try {
            doc.image(logoPath, left + 10, headerTop + 10, { width: 108 });
        }
        catch { }
    }
    doc
    ...
    if (cliRuc) {
        doc.font('Helvetica-Bold').text(`RUC/ID: ${cliRuc}`, left + 10, yLeft, { width: contentWidth / 2 - 12 });
        yLeft += extraStep;
    }
    if (cliEmpresa) {
        doc.font('Helvetica-Bold').text(`Empresa: ${cliEmpresa}`, left + 10, yLeft, { width: contentWidth / 2 - 12 });
        yLeft += extraStep;
    }
    if (cliContacto) {
        doc.font('Helvetica-Bold').text(`Contacto: ${cliContacto}`, left + 10, yLeft, { width: contentWidth / 2 - 12 });
        yLeft += extraStep;
    }
    if (cliRazon) {
        doc.font('Helvetica-Bold').text(`Razon Social: ${cliRazon}`, left + 10, yLeft, { width: contentWidth / 2 - 12 });
        yLeft += extraStep;
    }
    if (cliDir) {
        doc.font('Helvetica-Bold').text(`Direccion: ${cliDir}`, left + 10, yLeft, { width: contentWidth / 2 - 12 });
        yLeft += extraStep;
    }
    // Campos extra: derecha (Tel, Correo)
    let yRight = metaTop + 54;
    if (cliTel) {
        doc.font('Helvetica-Bold').text(`Tel: ${cliTel}`, left + contentWidth / 2, yRight, { width: contentWidth / 2 - 12, align: 'right' });
        yRight += extraStep;
    }
    if (cliMail) {
        doc.font('Helvetica-Bold').text(`Correo: ${cliMail}`, left + contentWidth / 2, yRight, { width: contentWidth / 2 - 12, align: 'right' });
    }
    if (hasMsg) {
    ...
    const drawTableHeader = (yHeader) => {
        ...
        const vTop = yHeader - 8;
        const vBot = yHeader + headerH - 8;
...
async function generarProformaPDFStreamV3({ empresa, cliente, detalles, totalCordoba, totalDolar, tipoCambio, pio }, res) {
    const doc = new pdfkit_1.default({ size: "A4", margin: 40 });
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="proforma.pdf"`);
    doc.pipe(res);
    ...
    const drawHeader = () => {
        ...
        if (logoPath) {
            try {
                doc.image(logoPath, left, headerTop, { width: 140 });
            }
            catch { }
        }
    ...
    const drawAtencion = (startY) => {
        ...
        if (linea2)
            doc.text(linea2, left, doc.y + 2);
        if (linea3)
            doc.text(linea3, left, doc.y + 2);
        return doc.y + 6;
    };
    const drawCondiciones = (startY) => {
        const lines = [
            { k: "INCOTERM:", v: "DDP NICARAGUA" },
            { k: "PLAZO DE ENTREGA:", v: "Inmediato" },
            { k: "CONDICION DE PAGO:", v: "30 dias credito" },
    ...
    const drawTabla = (startY) => {
        ...
        filas.forEach((d, i) => {
            ...
            doc.text(String(d.numeroParte ?? ""), colX[1] + 4, y + 4, { width: cols[1].w - 8 });
