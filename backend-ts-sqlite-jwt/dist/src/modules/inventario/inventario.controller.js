...
// ===============================
const CompetidorPrecioSchema = zod_1.z.object({
    proveedor: zod_1.z.string().min(1),
    precioCordoba: zod_1.z.number().optional(),
    precioDolar: zod_1.z.number().optional(),
    fecha: zod_1.z.string().optional(),
    referencia: zod_1.z.string().optional(),
});
const InvCreateSchema = zod_1.z.object({
    numeroParte: zod_1.z.string().min(1),
    ...
    ubicacion: zod_1.z
        ...
        .regex(/^[A-Z](?:[1-9]|1[0-2])$/)
        .optional(),
    stockActual: zod_1.z.number().int().optional().default(0),
    stockMinimo: zod_1.z.number().int().nonnegative().optional(),
    costoPromedioCordoba: zod_1.z.number().optional().default(0),
    precioVentaPromedioCordoba: zod_1.z.number().optional().default(0),
    precioVentaSugeridoCordoba: zod_1.z.number().optional().default(0),
    codigoSustituto: zod_1.z.string().optional().nullable(),
    marcaSustitutoId: zod_1.z.number().int().optional().nullable(),
    compatibilidadMaquinas: zod_1.z.array(zod_1.z.string()).optional(),
    preciosCompetencia: zod_1.z.array(CompetidorPrecioSchema).optional(),
});
// ===============================
// 💰 Función auxiliar: tipo de cambio
// ===============================
async function getTipoCambioDesdeAPI() {
    try {
    ...
    try {
        ...
        const itemsConvertidos = items.map((i) => ({
            ...
            compatibilidadMaquinas: Array.isArray(i.maquinasCompatibles)
                ? i.maquinasCompatibles.map((m) => m.nombre)
                : [],
            preciosCompetencia: Array.isArray(i.preciosCompetenciaRows)
                ? i.preciosCompetenciaRows.map((p) => ({
...
async function create(req, res) {
    try {
        ...
        // 🔍 Validar relaciones
        const marcaExiste = await prisma_1.prisma.marca.findUnique({ where: { id: data.marcaId } });
        if (!marcaExiste)
            return res.status(400).json({ message: "La marca especificada no existe." });
        const categoriaExiste = await prisma_1.prisma.categoria.findUnique({ where: { id: data.categoriaId } });
        if (!categoriaExiste)
            return res.status(400).json({ message: "La categoría especificada no existe." });
        if (data.marcaSustitutoId) {
            const marcaSust = await prisma_1.prisma.marca.findUnique({ where: { id: data.marcaSustitutoId } });
            if (!marcaSust)
        ...
        if (typeof createData.ubicacion === 'string') {
            createData.ubicacion = createData.ubicacion.toUpperCase();
        }
        ...
        if (Object.prototype.hasOwnProperty.call(data, "compatibilidadMaquinas")) {
            const arr = Array.isArray(data.compatibilidadMaquinas)
                ? data.compatibilidadMaquinas
                : [];
            createData.maquinasCompatibles = {
                create: arr.map((nombre) => ({ nombre })),
            };
        ...
        if (Object.prototype.hasOwnProperty.call(data, "preciosCompetencia")) {
            const arr = Array.isArray(data.preciosCompetencia)
                ? data.preciosCompetencia
                : [];
        ...
        try {
            ...
            const unknownArg = msg.includes("Unknown argument `maquinasCompatibles`") ||
...
async function getById(req, res) {
    try {
        ...
        const tipoCambio = await getTipoCambioDesdeAPI();
        const item = await prisma_1.prisma.inventario.findUnique({
            where: { id },
            include: {
                marca: true,
                categoria: true,
        ...
        const convertido = {
            ...item,
            compatibilidadMaquinas: Array.isArray(item.maquinasCompatibles)
                ? item.maquinasCompatibles.map((m) => m.nombre)
                : [],
            preciosCompetencia: Array.isArray(item.preciosCompetenciaRows)
                ? item.preciosCompetenciaRows.map((p) => ({
                    proveedor: p.proveedor,
                    precioCordoba: p.precioCordoba != null ? Number(p.precioCordoba) : undefined,
                    precioDolar: p.precioDolar != null ? Number(p.precioDolar) : undefined,
                    fecha: p.fecha || undefined,
...
async function update(req, res) {
    try {
        ...
        const productoExiste = await prisma_1.prisma.inventario.findUnique({ where: { id } });
        if (!productoExiste)
            return res.status(404).json({ message: "Producto no encontrado." });
        // 🔍 Validar relaciones
        if (data.marcaId) {
            const marca = await prisma_1.prisma.marca.findUnique({ where: { id: data.marcaId } });
            if (!marca)
                return res.status(400).json({ message: "La marca especificada no existe." });
        }
        if (data.categoriaId) {
            const cat = await prisma_1.prisma.categoria.findUnique({ where: { id: data.categoriaId } });
            if (!cat)
                return res.status(400).json({ message: "La categoría especificada no existe." });
        }
        if (data.marcaSustitutoId) {
        ...
        if (typeof updateData.ubicacion === 'string') {
            updateData.ubicacion = updateData.ubicacion.toUpperCase();
        }
        ...
        if (Object.prototype.hasOwnProperty.call(data, "compatibilidadMaquinas")) {
            ...
            updateData.maquinasCompatibles = {
                deleteMany: {},
                ...(arr.length > 0 ? { create: arr.map((nombre) => ({ nombre })) } : {}),
            };
            // Remove input-only key to avoid Prisma unknown argument
...
// ===============================
async function asignarUbicaciones(_req, res) {
    try {
        ...
        const usadosRows = await prisma_1.prisma.inventario.findMany({
        ...
        const porAsignar = await prisma_1.prisma.inventario.findMany({
        ...
        await prisma_1.prisma.$transaction(asignaciones.map((a) => prisma_1.prisma.inventario.update({ where: { id: a.id }, data: { ubicacion: a.ubicacion } })));
        res.json({
            ...
            aviso: porAsignar.length > asignaciones.length
...
async function remove(req, res) {
    try {
        const id = Number(req.params.id);
        await prisma_1.prisma.inventario.delete({ where: { id } });
        res.json({ ok: true });
    }
    catch (err) {
...
// ===============================
async function viewConSustituto(_req, res) {
    try {
        const rows = await prisma_1.prisma.inventario.findMany({
            select: {
                id: true,
                numeroParte: true,
                stockActual: true,
...
// ===============================
async function buscarProductoDisponible(req, res) {
    try {
        const numeroParte = String(req.query.numeroParte ?? "");
        const marcaId = Number(req.query.marcaId ?? 0);
        const i = await prisma_1.prisma.inventario.findFirst({
