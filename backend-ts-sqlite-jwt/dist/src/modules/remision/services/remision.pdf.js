...
const fmtMoney = (n, symbol) => `${symbol} ${Number(n || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
const pickPrecioCordoba = (inv) => Number(inv?.precioVentaSugeridoCordoba ?? inv?.precioVentaPromedioCordoba ?? 0) || 0;
const pickPrecioDolar = (inv, tipoCambio) => {
    const directo = Number(inv?.precioVentaSugeridoDolar ??
        inv?.precioVentaPromedioDolar ??
        inv?.precioDolar ??
        0) || 0;
    if (directo > 0)
        return directo;
    const cordoba = pickPrecioCordoba(inv);
    if (tipoCambio && tipoCambio > 0)
        return cordoba / tipoCambio;
    return 0;
};
async function generarRemisionPDFStreamV2({ empresa, cliente, detalles, numero, fecha, observacion, pio, entregadoA, tipoCambio, }, res) {
    const doc = new pdfkit_1.default({ size: "A4", margin: 28 });
    ...
    const drawHeader = () => {
        ...
        if (logoPath) {
            try {
                doc.image(logoPath, left, 24, { width: 150 });
            }
            catch { }
        }
    ...
    const drawDatosGenerales = (startY) => {
        ...
        rows.forEach((r, idx) => {
            ...
            if (idx % 2 === 0)
                doc.rect(left, y, contentWidth, h).fillOpacity(0.06).fill(colorAccent).fillOpacity(1);
    ...
    const drawItemsTable = (startY) => {
        ...
        if (descW < 110)
            descW = 110;
        if (descW + fixedWidth > contentWidth)
            descW = contentWidth - fixedWidth;
        const cols = [
            { title: "COD", w: codeW },
        ...
        for (let i = 1; i < cols.length; i++)
            doc.moveTo(colX[i], startY).lineTo(colX[i], startY + headerH).stroke();
        doc.restore();
        ...
        filas.forEach((d, idx) => {
            ...
            const precioC = pickPrecioCordoba(d.inventario);
            const precioU = pickPrecioDolar(d.inventario, tipoCambio);
        ...
        for (let i = 1; i < cols.length; i++)
            doc.moveTo(colX[i], y).lineTo(colX[i], y + totalRowH).stroke();
        doc.restore();
